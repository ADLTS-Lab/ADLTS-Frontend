...
import { CandidateSettings, getCandidateSettings, updateCandidateSettings } from "@/services/settings.service";
...
export default function CandidateSettingsPage() {
  ...
  useEffect(() => {
    let isMounted = true;
    getCandidateSettings().then((data) => {
      if (isMounted) {
        setSettings(data);
        setIsLoading(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updated = await updateCandidateSettings(settings);
      setSettings(updated);
  ...
  const updateNestedSetting = (key: keyof CandidateSettings['notifications'], value: boolean) => {
    setSettings((prev) => {
      if (!prev) return prev;
      return {
        ...prev,
  ...
  return (
    <main className="max-w-4xl mx-auto space-y-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-slate-500 text-sm">Manage your portal preferences and notifications.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-100">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-2">Note: Changing the language here simulates updating your preference. Use the language toggle in the header for immediate effect.</p>
          </div>
        </div>
              ...
              {["light", "dark", "system"].map((theme) => (
                <label key={theme} className="flex items-center gap-3 cursor-pointer p-4 border border-slate-200 rounded-xl hover:bg-slate-50 transition w-full sm:w-auto">
                  <input
                    ...
                    onChange={(e) => setSettings({ ...settings, theme: e.target.value as CandidateSettings["theme"] })}
          ...
          <div className="space-y-4">
            <label className="flex items-center gap-3 p-3 hover:bg-slate-50 rounded-lg cursor-pointer transition">
              <input
                ...
                onChange={(e) => updateNestedSetting('email', e.target.checked)}
                className="w-5 h-5 rounded text-blue-600 focus:ring-blue-500 border-slate-300"
              />
              <span className="text-sm font-medium text-slate-700">Email Notifications (Updates, Booking statuses)</span>
            </label>
            <label className="flex items-center gap-3 p-3 hover:bg-slate-50 rounded-lg cursor-pointer transition">
              <input
                type="checkbox"
                checked={settings.notifications.sms}
                onChange={(e) => updateNestedSetting('sms', e.target.checked)}
                className="w-5 h-5 rounded text-blue-600 focus:ring-blue-500 border-slate-300"
              />
              <span className="text-sm font-medium text-slate-700">SMS Notifications (Urgent alerts)</span>
            </label>
            <label className="flex items-center gap-3 p-3 hover:bg-slate-50 rounded-lg cursor-pointer transition">
              <input
                type="checkbox"
                checked={settings.notifications.examReminders}
                onChange={(e) => updateNestedSetting('examReminders', e.target.checked)}
                className="w-5 h-5 rounded text-blue-600 focus:ring-blue-500 border-slate-300"
              />
              <span className="text-sm font-medium text-slate-700">Exam Reminders (24h before exam)</span>
            </label>
          </div>
        </div>
