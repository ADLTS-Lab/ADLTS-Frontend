...
export default function SuperAdminCandidatesPage() {
  ...
  useEffect(() => {
    const loadCandidates = async () => {
      ...
      try {
        setCandidates(await listCandidates(search ? { search } : undefined));
      } catch (err: unknown) {
  ...
  return (
    <PageContainer width="wide" className="space-y-6">
      <PageHeader
        ...
        description="Review candidate accounts and manage active or suspended status with the current identity APIs."
      />
      ...
      <Card padding="md">
        <CardHeader
          ...
          description="Search candidate records before reviewing or changing account status."
        />
        <Input
          value={search}
